...
impl App {
    ...
    fn render_main(&self, frame: &mut Frame, area: Rect) {
        ...
        let [charging_panel_area, privacy_panel_area] =
            Layout::horizontal([Constraint::Min(0), Constraint::Min(0)]).areas(main_area);
    ...
    fn render_charge_level(&self, frame: &mut Frame, key_area: Rect, value_area: Rect) {
        ...
        let status = match (is_charging, is_ac_connected) {
            (true, true) => "Charging",
        ...
        let gauge = match self.framework.controls.charge_percentage() {
            Some(charge_percentage) => {
                ...
                let label = format!("{} {}%", status, charge_percentage);
    ...
    fn render_privacy_panel(&self, frame: &mut Frame, area: Rect) {
        let block = Block::default().title("Privacy").borders(Borders::ALL);
